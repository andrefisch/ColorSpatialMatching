...
public class PlaygridController : MonoBehaviour {
    ...
    */
    ...
    void HighlightMatchTrack()
    {
        ...
        line.SetVertexCount(matchTrack.Count + 1);
        line.numPositions = (matchTrack.Count + 1);
        if (DEHIGHLIGHTMATCHTRACK)
        {
        ...
        for (int z = 1; z < matchTrack.Count; z++)
        {
            ...
            Vector3 position1;
            Vector3 position2;
            // Create a line between this block and previous block in list if they are adjacent
            if (CheckAdjacency((int)matchTrack[z].x, (int)matchTrack[z].y, (int)matchTrack[z - 1].x, (int)matchTrack[z - 1].y))
            {
                if (DEHIGHLIGHTMATCHTRACK)
                {
                    Debug.Log((int)matchTrack[z].x + ", " + (int)matchTrack[z].y + " and " + (int)matchTrack[z - 1].x + ", " + (int)matchTrack[z - 1].y + " are adjacent");
                }
                if (gridObjects[(int)matchTrack[z].x, (int)matchTrack[z].y] && gridObjects[(int)matchTrack[z - 1].x, (int)matchTrack[z - 1].y])
                {
                    target1 = gridObjects[(int)matchTrack[z].x, (int)matchTrack[z].y].GetComponent<Transform>();
                    target2 = gridObjects[(int)matchTrack[z - 1].x, (int)matchTrack[z - 1].y].GetComponent<Transform>();
            ...
            else
            {
                if (DEHIGHLIGHTMATCHTRACK)
                {
                    Debug.Log((int)matchTrack[z].x + ", " + (int)matchTrack[z].y + " and " + (int)matchTrack[z - 1].x + ", " + (int)matchTrack[z - 1].y + " are NOT adjacent");
                }
            }
        }
    }
